...
| `imageAlt`                 | **Required.** German description of what the picture shows, for people who cannot see it. Describe the picture, never the credit; include any text that is part of the image.                                                                                                                                                                                                            |
| `imageAttribution`         | Human-readable credit string shown in the app footer - not the licensing record (that's §4). Never used as the image's `alt` text; that is `imageAlt`.                                                                                                                                                                                                                                   |
| `sourceLabel`, `sourceUrl` | Optional "Quelle" link shown under the body (e.g. a linked article/app).                                                                                                                                                                                                                                                                                                                 |
| `relatedSources`           | Optional array of `{ title, url }` "Mehr zum Thema" links, shown below the body. Publisher/domain is derived from `url` automatically - omit the field entirely when there are none. **Only ever link what Amazone supplied with the entry**; never research a link (a Wikipedia article, say) and add it yourself - the app then vouches for a page nobody in the editorial team chose. |
...
| `dailyRender`              | Optional `'image'` or `'teaser'` (default). How the Today card renders this item: `image` leads with the picture, `teaser` leads with the teaser line. Pick `image` only when the picture says more than the teaser does.                                                                                                                                                                |
...
- No SQLite sync. `SupportServiceCatalogGateway` fetches the file directly on every page load; there
  is no `content_items` table and no build step, and editing the file is the whole update path.
  The file does carry a top-level `version`, but nothing reads it - unlike the Wissensimpulse
  catalog's, it is a bookkeeping note for content review, and forgetting it breaks nothing.
  Anlaufstellen has no per-item state to persist (no bookmarking, no daily rotation, no read
  history), so the `ContentCatalogSync` machinery the Wissensimpulse/Rebell\*in catalog needs would
  add nothing here. Fields per entry:
...
| `icon`           | One emoji, freely chosen to fit the service (e.g. 🧠, 🛡️, ⚖️) - drawn plain as the card's lead visual, with no frame behind it. Every entry needs one; there's no default and no shared palette to pick from. It must be **unique within its region**: the filter is single-select, so one region's list is the whole visible set. But **one organisation keeps one emoji across regions** - an Anlaufstelle with an online offer and a Vorarlberg office uses the same emoji for both. Two offers of the same organisation inside one region may share an emoji too; list the pair in `SHARED_EMOJI_BY_DESIGN` so the guard spec below allows it. |
...
`support-services.catalog-content.spec.ts` reads the committed file and enforces what the gateway's
runtime type guards cannot see: unique kebab-case ids, known regions, non-empty trimmed text,
emoji unique per region, a `uri` scheme matching each action's type, and a `displayValue` whose
digits are the digits the `uri` dials. It runs as part of `pnpm test:ci`. Both exception lists in
it - `SHARED_EMOJI_BY_DESIGN` and `HTTP_ONLY_BY_NECESSITY` - name the entry and the reason; add to
them deliberately, never to silence a failure.
